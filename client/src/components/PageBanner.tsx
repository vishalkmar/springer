    ...
    <section className="relative h-[40vh] md:h-[50vh] lg:h-[60vh] flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 text-center text-white">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          <h1 className="text-4xl md:text-6xl lg:text-8xl font-bold font-display mb-6 drop-shadow-2xl uppercase tracking-tighter leading-none">
            {title}
          </h1>
          {subtitle && (
            <div className="bg-white/10 backdrop-blur-md px-8 py-3 rounded-full inline-block border border-white/20">
              <p className="text-sm md:text-lg font-medium tracking-widest uppercase">{subtitle}</p>
            </div>
          )}
        </motion.div>
      </div>
